...
export default function GeoHotspotSection() {
  const [isZoomed, setIsZoomed] = useState(false);
  const [imageError, setImageError] = useState(false);

  const imageUrl = `https://inderaja.bmkg.go.id/IMAGE/GEOHOTSPOT/H08_GH_Kaltim.png?id=${Date.now()}`;
    ...
    <div className="bg-white rounded-xl shadow-md p-3 flex flex-col h-full">
      ...
      <div className="bg-blue-50 text-gray-700 rounded-xl p-4 mt-4 border border-blue-100">
        <p className="text-sm sm:text-base leading-relaxed">
          <strong>Geo Hotspot</strong> menggambarkan area kebakaran hutan 
          dan lahan dapat teramati dengan citra satelit Himawari-9 dengan menggunakan data 
          suhu kecerahan kanal infrared untuk filtering awan, serta menentukan anomali suhu 
          panas yang menunjukkan potensi terjadi kebakaran hutan (titik merah).Selain itu 
          ditampilkan juga citra RGB pada kanal visibel dan near infrared untuk mendeteksi 
          sebaran asap (warna coklat) untuk lebih memastikan didaerah tersebut terjadi kebakaran.
        </p>
      </div>
